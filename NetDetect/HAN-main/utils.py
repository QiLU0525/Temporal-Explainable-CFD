...
def load_acm(remove_self_loop):
    ...
    labels = labels.nonzero()[:, 1] # 所有非0元素的列标

    if remove_self_loop:
        num_nodes = data["label"].shape[0] # 节点的个数，即label的行数
        # np.eye: [num_nodes, num_nodes] 的对角矩阵
...
def load_data(dataset, remove_self_loop=False):
    if dataset == "ACM":
        # 非hetero, HAN论文里的数据集ACM
        return load_acm(remove_self_loop)
    elif dataset == "ACMRaw":
        # hetero, dgl自己的数据集
        return load_acm_raw(remove_self_loop)
    elif dataset == 'CorpNet':
        return load_corp_net()
    else:
