...
# 2023.06.02，用25条路径，重新生成ChinaCorp_1x_20230602.pt.

KGE_path = '../../data/KGE_pytorch/source/embed/entityEmbedding_{}.npy'
metapath_dir = '../../data/meta-path/mat_by_year/{}/metapath/{}.mtx'
# 25 条路径， 文件夹里面的没有全部弄
metapath_list = [
    # 1-hop
    'guanlian', 'gudong',
...
def setup_log_dir(args):
    ...
    log_dir = os.path.join(
        args["log_dir"], "{}_{}_{}_{}_{}_{}_{}_{}_{}_{}_{}_{}".format(
            args["model"], args["lr"], args["dropout"], args["weight_decay"], args["num_heads"], args["batch_size"], args["num_epochs"], args["MAX_SEQ"], args["rnn_hidden"], args["node_emb_size"], args["emb_init"], date_postfix)
    )
    mkdir_p(log_dir)
    return log_dir
...
def generate_graph(year, listed_corp):
    ...
    raw_fraud = pd.read_csv('../../data/frau_time_industry_money_til2022.csv', encoding='gb18030')
    ...
    graph['path_mask'] = [] # [4111, 25]

    for g in graph['adj']:
        out_degrees = g.out_degrees().int()
        in_degrees = g.in_degrees().int()
        has_degree = ((out_degrees + in_degrees) != 0).int()
        # has_degree
        graph['path_mask'].append(has_degree.unsqueeze(1))

    graph['path_mask'] = torch.cat(graph['path_mask'], dim=1)
...
def load_corp_net():
    # 生成ChinaCorp.pt数据集
    MAX_POS_NEG_RATE = 1
    all_data = pd.read_csv('../../data/train_test_x{}.csv'.format(MAX_POS_NEG_RATE), encoding='gb18030')
    ...
    '''读取financial ratio'''
    fin_ratios = pd.read_excel('../../data/会计信息质量-财务指标/findata_normalized.xlsx').set_index(['Symbol', 'EndDate'],inplace=False)
    nonfin_ratios = pd.read_csv('../../data/非财务指标/nonfin_data_v2.csv', encoding='gb18030').set_index(['Symbol', 'year'], inplace=False)
    mda = pd.read_excel('../../data/年报/管理层讨论与分析/mda_no_number.xlsx').set_index(['Scode', 'Year'], inplace=False)
    ...
    op_dict = {}
    op_dict['Stkcd_year'] = torch.from_numpy(all_data[['Stkcd','pred_year']].values).long().squeeze(0)
    ...
    torch.save(op_dict, f'ChinaCorp_{MAX_POS_NEG_RATE}x_{date}.pt'.format())
