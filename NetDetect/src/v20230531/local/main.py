...
def main(args):
    ...
    model = Model(
        ...
        num_path=len(gs[2003]['adj']), # 每一年的path的条数都是一样的
    ...
    for epoch in range(args["num_epochs"]):
        ...
        with open(os.path.join(args["log_dir"],'train.log'), 'a+') as f:
            f.write("Epoch: {}\n".format(epoch))
        ...
        auc, acc, fr_prec, fr_recall, le_prec, le_recall, macro_f1, micro_f1, pos_neg_rate = \
            model.score(torch.cat(logits,dim=0), fraud_labels[train_idx])
        train_results = {
            'loss': round(mean_losses, 4),
            'auc': auc, 'fr_prec': fr_prec, 'fr_re': fr_recall,
            'le_prec': le_prec, 'le_re': le_recall,
            'mac_f1': macro_f1, 'mic_f1': micro_f1, 'acc': acc,
        ...
        if epoch % args["valid_epoch"] == 0:
            loss, auc, acc, fr_prec, fr_recall, le_prec, le_recall, macro_f1, micro_f1, pos_neg_rate = \
                model.evaluate_(node_seq[valid_idx],
                                fin_seq[valid_idx],
            ...
            # 用于保存模型，生成 config.json
            early_stop = logger.step(loss=loss.item(), acc=acc, model=model, epoch=epoch, optimizer=optimizer)
    ...
    best_test_results = {
        ...
        'auc': auc, 'fr_prec': fr_prec, 'fr_re': fr_recall,
        'le_prec': le_prec, 'le_re': le_recall,
        'mac_f1': macro_f1, 'mic_f1':micro_f1 , 'acc': acc, 'pos_neg': pos_neg_rate
    }
...
if __name__ == "__main__":
    ...
    parser.add_argument("--valid_epoch", type=int, default=3, help="valid epoch")
    ...
    parser.add_argument('--use_kge', type=bool, default=True, help='encode node simply by TransD')
    ...
    parser.add_argument('--use_conven_feat', type=bool, default=False, help='use conventional features')
    parser.add_argument('--path', type=str, default='', help='include which path')
    default_configure = {
        "lr": 1e-4,
        "num_heads": [1, 1],  # Number of attention heads for node-level attention
        # "hidden_units": 32,
        "dropout": 0.0,
        "weight_decay": 0.001,
        "num_epochs": 20,
