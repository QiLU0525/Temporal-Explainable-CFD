...
def main(args):
    mda_bert_embs = torch.load('../../data/mda_embs.pt')
    corp_data = torch.load('../../data/ChinaCorp_{}x_2023-06-07.pt'.format(args['MAX_POS_NEG_RATE']))
    ...
    model = Model(
        ...
        out_size = 2,
        num_path = 25 if args["path"]=='total' else 1,
    ...
    for epoch in range(args["num_epochs"]):
        ...
        if args['use_gat']:
            ...
            for i in range(len(gs)): # 19 个自然年
                # 把 25 个 meta path图合并，先转换成 scipy->numpy
            ...
            for i in range(len(gs)): # 19 个自然年
                ...
                node_indices = torch.tensor(range(i * n_listed_corps, (i + 1) * n_listed_corps)).to(args["device"])
                path_mask = gs[2003 + i]['path_mask'].to(args["device"])
                HAN, logits, path_attn, gat_attn = model.hans_infer(i, adj, node_indices, path_mask)
                # model.py 里面已经 detach 把梯度清了，再转 cpu和numpy
                if 'path' in args['print_param']:
                    path_attns.append(path_attn.cpu().numpy())
                if 'gat' in args['print_param']:
                    gat_attns.append([coo_mat.cpu() for coo_mat in gat_attn])
        ...
        for i, (n_seq, f_seq, nf_seq, m_seq, seq_len, label) in enumerate(train_loader):
            logit, _ = model(n_seq, f_seq, nf_seq, m_seq, seq_len)
        ...
        if epoch % args["valid_epoch"] == 0:
            # model.eval()
            val_results, _, _ = model.evaluate_(node_seq[valid_idx],
                                fin_seq[valid_idx],
            ...
            # test stage
            test_results, mlp_input, logits = model.evaluate_(
                    node_seq[test_idx],
                    fin_seq[test_idx],
                    nonfin_seq[test_idx],
                    mda_seq[test_idx],
            ...
            if logger.best_epoch == epoch:
                # 当前轮次更新为最佳轮次
                if 'path' in args['print_param']:
                    # 保存 path_attns [19, 4111, 25]
                    path_attns = np.stack(path_attns)
                    np.save(f'{args["log_dir"]}/path_attn_{args["seed"]}.npy', path_attns)
                    # 保存 path_attns 列表: [19, 25, 4111, 4111]
                if 'gat' in args['print_param']:
                    with open(f'{args["log_dir"]}/gat_attn_{args["seed"]}.pt', 'wb') as f:
                        # 可以只存 20年和21年的 gat attn
                        pickle.dump(gat_attns[-2:], f)
                if 'pred' in args['print_param']:
                    # 保存预测的 logits: [-1, 2]
                    np.savetxt(f'{args["log_dir"]}/y_pred_{args["seed"]}.txt', logits)
                if 'mlp' in args['print_param']:
                    # 保存mlp_input，用于做t-SNE
                    np.save(f'{args["log_dir"]}/output_{args["seed"]}.npy', mlp_input)
    ...
    # best test
    # model.eval()
    best_test_results, _, _ = model.evaluate_(
            node_seq[test_idx],
            fin_seq[test_idx],
            nonfin_seq[test_idx],
            mda_seq[test_idx],
