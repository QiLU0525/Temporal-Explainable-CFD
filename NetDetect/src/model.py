...
class HANLayer(nn.Module):
    ...
    def forward(self, gs, h, path_mask):
        semantic_embeddings = []
        # 将 25 个图的 gat_attn 顺序拼接起来, [25, 4111, 4111]
        gat_attns = []
        # gat_edge_nums = [] # 25 个图每一个的 edge num 存起来
        for i, g in enumerate(gs):
            # i = 1, 2, 3,...25, 25个 meta-path 的图
            # 一个gat_layer 是一个 GAT 网络
            # 给邻接矩阵 g 加上一个对角矩阵，让每个节点和自己相连，否则 GATConv 不允许有度（degree）为 0 的点
            g = add_self_loop(g)
            h = self.LN(h)
            # 每一个 gat_attn 的shape 不一样: [num_edges, num_heads, 1]
            g_emb, gat_attn = self.gat_layers[i](g, h, get_attention=True)

            # [edge_num] -> [4111, 4111]
            gat_attn = gat_attn.mean(1).squeeze()
            gat_attn = self.attn_coef_to_mat(g, gat_attn)
            gat_attns.append(gat_attn.detach())
            semantic_embeddings.append(g_emb.flatten(1))
        ...
        # path_attn: (4111, 25, 1)
        sum_embeddings, path_attn = self.graph_sum(semantic_embeddings, path_mask)
    ...
    def attn_coef_to_mat(self, g, gat_attn):
        src, dst = g.edges()
        # adj_mat = dgl.graph((src, dst), num_nodes=4111)
        # adj_mat.edata['weight'] = gat_attn
        # adj_mat = adj_mat.adjacency_matrix().to_dense()
...
class HAN(nn.Module):
    ...
    def forward(self, g, h, path_mask):
        path_attns = []
        gat_attns = []
        for gnn in self.layers:
            # 有2层 HAN 就有2层 attn_weights: [4111, 25, 1]
            h, p_attn, g_attn = gnn(g, h, path_mask)
            path_attns.append(p_attn)
            gat_attns.append(g_attn)
            h = self.BN(h)

        # path_attn: [2, [4111, 25, 1]] -> [4111, 25, 2]
        # path_attn = torch.cat(path_attns, dim=-1)

        # 只取第一层 HAN 的attn_weight, [4111, 25]
        return h, self.mlp(h), path_attns[0].squeeze(-1), gat_attns[0]
...
class Model(nn.Module):
    ...
    # 输出hans modules
    def hans_infer(self, i, adjs, indices, path_mask):
        # 第 i 年，第 i 个图
        # node_embs = self.han_LN(self.node_embs(indices))
        node_embs = self.node_embs(indices)

        output, logits, path_attns, gat_attns = self.HANs[i](adjs, node_embs, path_mask)

        self.han_outputs.weight.data[i*self.n_entities: (i+1)*self.n_entities].copy_(output)
        return self.HANs[i], logits, path_attns, gat_attns
    ...
    def evaluate_(self, node_seq, fin_seq, nfin_seq, mda_seq,  seq_len, labels):
        ...
        return output_dict, output.cpu().numpy(), logits.cpu().numpy()
